...
class DataInterface(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.trainset = self.instancialize(ds_type='train', sample_size=self.num_train)
            self.valset = self.instancialize(ds_type='valid', sample_size=self.num_valid)

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            self.testset = self.instancialize(ds_type='test', sample_size=self.num_test)

        if stage == 'predict' or stage is None:
            self.predictset = self.instancialize(ds_type='test', sample_size=self.num_test)
